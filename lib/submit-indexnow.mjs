...
try {
  ...
  if (response.status !== 200) {
    const details = response.status === 400 || response.status === 403 || response.status === 422 || response.status === 429
      ? `IndexNow rejected the batch with HTTP ${response.status}. Verify the key file, host, payload, and rate limits.`
      : `IndexNow returned HTTP ${response.status}; only HTTP 200 is treated as successful submission.`
    throw new Error(details)
  }

  console.log(`Accepted ${urlList.length} URL(s) for processing. Submission does not guarantee indexing.`)
} catch (error) {
